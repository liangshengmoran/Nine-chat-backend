...
**Nine-Chat** 是一个功能完整的实时在线音乐聊天室平台，基于 NestJS + Vue 3 构建。用户可以创建房间、多人实时聊天、共享音乐播放。平台提供了完善的 Bot 机器人生态、动态 RBAC 权限系统，以及全功能管理后台。

### 主要特色

- **前后端分离**：后端 NestJS RESTful API + Socket.IO，前端 Vue 3 SPA
...
| 分类 | 技术 | 版本 | 说明 |
|------|------|------|------|
| 框架 | Vue | 3.5.x | 渐进式前端框架（Composition API） |
| UI 组件 | Element Plus | 2.9.x | 桌面端组件库（按需引入） |
| 状态管理 | Pinia | 2.3.x | 模块化状态管理 |
| 路由 | Vue Router | 4.5.x | 前端路由 |
| HTTP | Axios | 1.7.x | HTTP 请求库 |
| 构建 | Vite | 6.1.x | 新一代前端构建工具 |
| 代码检查 | ESLint | 9.x | 代码规范检查 |
